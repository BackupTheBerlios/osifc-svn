...
void init_Powr(void)
{
	...
	AD0CR =  (0 << 21); 			//has to be set to 0 before switching ADC power
									...
									//PCONP 0 is unused and should not ben set
...
void init_sspi0(void)
{
	...
	PCLKSEL1  	|= NBIT(11) | BIT(10);
	SSP0CR0 	|= 0x5007; 				//set to 8 Bit(0111),FRF = 0, CPOL = 0, CPHA = 0, SCR = 0x50
	SSP0CR0 	|= NBIT(4) | NBIT(5); 	//SPI Mode
	SSP0CPSR 	= 0x02; 				//prescaler should be Minimum 2 as bit 0 is always 0 by hardware
...
void init_PWM(void)
{
	...
	case PWM_MC:
		...
		//P2.6 to Capture
		//P2.7 to Capture MM3 DRDY
		IO2_INT_EN_R |= pwmin0_BIT | pwmin8_BIT;
