...
int PWMNick = 0;
int PWMRoll = 0;
unsigned int PWMChannelCounter = 0;
...
void setPWMChannelData(void)
{
	if (++PWMChannelCounter <= 50) {
		PWMNickChannelDataMux += PWM_channel[fcSetup.nickServoChan];
		PWMRollChannelDataMux += PWM_channel[fcSetup.rollServoChan];
	} else {
		PWMNickChannelData = PWMNickChannelDataMux / PWMChannelCounter;
		PWMRollChannelData = PWMRollChannelDataMux / PWMChannelCounter;
	}
}
...
void setServo(void)
{
	...
	PWMNick = (unsigned int)PWMNick+nickPos;
	PWMRoll = (unsigned int)PWMRoll+rollPos;
	...
	if(PWMNick > pwmMaxLength)
	{
		PWMNick = pwmMaxLength;
	}
	if(PWMNick < pwmMinLength)
	{
		PWMNick = pwmMinLength;
	}

	if(PWMRoll > pwmMaxLength)
	{
		PWMRoll = pwmMaxLength;
	}
	if(PWMRoll < pwmMinLength)
	{
		PWMRoll = pwmMinLength;
	}
...
//for now there is no further PWM implementation.
//that will come with the servo controls
