...
signed int atan2_i(signed int x, signed int y)
{
	...
	i = (unsigned int)((unsigned long)(yy*64) / xx);		//Berechne i für die Lookup table (Schrittweite atan(x) ist 0,015625 -> y *64)
...

{
	 ...
	 if (angel <= 90) n=1;
	 else if ((angel > 90) && (angel <= 180)) {angel = 180 - angel; n = 1;}
	 else if ((angel > 180) && (angel <= 270)) {angel = angel - 180; n = -1;}
	 else {angel = 360 - angel; n = -1;}	//if ((winkel > 270) && (winkel <= 360))

	 return ((signed int)((lut_sinus_i[angel]*m*n)/1000));
}
